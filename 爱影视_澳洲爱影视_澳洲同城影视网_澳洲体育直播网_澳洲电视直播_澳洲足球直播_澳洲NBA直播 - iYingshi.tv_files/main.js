...
function bannerSlide() {
	...
	var tab = function(cur, next) {
		...
		$('.ag7-bannerslide .slidelist li').removeClass('cur');
		$('.ag7-bannerslide .slidelist li').eq(next).addClass('cur');
		console.dir(cur);
		mask.stop(true, true).animate({
			top: buttons.find('li').eq(next).position().top
		}, 300, 'easeOutExpo');
		setTitle(next);
		curIndex = next;
	}
